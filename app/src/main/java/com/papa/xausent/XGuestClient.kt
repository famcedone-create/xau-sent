...
object XGuestClient {
    ...
    fun fetchWindows(context: Context): XWindows {
        return try {
            ...
            XWindows(five, summarizeFlow(recent), ten)
        } catch (_: Throwable) {
    ...
    private fun unavailableWindows(accountsOk: Int, timelinesOk: Int, configured: Int, statuses: Map<String, String>) = XWindows(
        FlowSummary(), FlowSummary(), XFlowData("non disponibile", accountsOk = accountsOk, timelinesOk = timelinesOk, accountsConfigured = configured, accountStatuses = statuses)
    )
    ...
    data class XWindows(val five: FlowSummary, val tenSummary: FlowSummary, val ten: XFlowData)
